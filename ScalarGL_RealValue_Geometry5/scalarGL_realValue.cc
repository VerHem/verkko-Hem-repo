...
namespace ScalarGL
{
  ...
  void RealValuedScalarGLSolver<dim>::run()
  {
    ...
    const Point<dim> bottom_left(-25., -15., -5);
    const Point<dim> top_right(25., 15, 5);

    const std::vector<unsigned int> repititions = {6, 5, 5};
    const std::vector<int> n_cells_to_remove = {-4, -4, 5};
	...
	std::cout << "Mesh refinement step " << refinement_cycle
	    ...
	    last_residual_norm = system_rhs.l2_norm();
	    if ((refinement_cycle !=0) && (last_residual_norm < 9e-3))
	      break;
          }
