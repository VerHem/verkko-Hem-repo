...
def rhs(x, y):
    ...
    return np.vstack((y[1], -2.*y[0] + y[0]*y[0]*y[0]))
...
def bc(ya, yb):
    # only for t = 0.5, yb[0] = np.sqrt(2)
    return np.array([ya[0], yb[0] - np.sqrt(2)])
...
with open('digtialized_realGLeq_L_shape_t05.csv', newline='') as f:
     reader = csv.reader(f)
     data = list(reader)
...
plt.title('FEM of 2D L-shape geometry, 1D Python bvp solver, 1D analytic solution on $t=0.5$', fontsize=20)
plt.show()
