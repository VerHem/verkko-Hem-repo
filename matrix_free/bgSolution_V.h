...
#ifndef BGSOLUTION_V_H
#define BGSOLUTION_V_H
...
namespace VerHem
{
  ...
  /* ------------------------------------------------------------------------------------------
   * class template noised B-phase imagnary part V inhereted from Function<dim>.
   * set the reference value_list to noised B-phase configuration for full-step newton iteration.
   * ------------------------------------------------------------------------------------------
   */
  ...
  public:
                              ...
                              Vector<Number> &values) const override
    {
      ...
      {
          values[0] = gaussian_dis(gen);  /*v11*/  
          values[1] = gaussian_dis(gen);  /*v12*/  
          values[2] = gaussian_dis(gen);  /*v13*/  
 
          values[3] = gaussian_dis(gen);  /*v21*/ 
          values[4] = gaussian_dis(gen);  /*v22*/ 
          values[5] = gaussian_dis(gen);  /*v23*/ 

          values[6] = gaussian_dis(gen);  /*v31*/ 
          values[7] = gaussian_dis(gen);  /*v32*/ 
          values[8] = gaussian_dis(gen);  /*v33*/ 
      }      
                      ...
                      std::vector<Vector<Number>> &  value_list) const override
    {
      ...
      for (unsigned int p = 0; p < points.size(); ++p)
        bgSolution_V<dim, Number>::vector_value(points[p], value_list[p]);
    }
  }; // bgSolution_V class ends here
