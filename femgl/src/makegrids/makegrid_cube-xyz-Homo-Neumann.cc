  ...
  void FemGL<dim>::make_grid()
  {
	...
	for (const auto &cell : triangulation.cell_iterators())
	  for (const auto &face : cell->face_iterators())
		...
		face->set_boundary_id(1); // b_id1 face of periodic BC along x direction 

	      if (
		  (std::fabs(center(0) - half_length) < 1e-12 * half_length)
		 )
		face->set_boundary_id(1); // b_id2 face of periodic BC along x direction 
	      
	      if (
		  (std::fabs(center(1) - (-half_length)) < 1e-12 * half_length)
		 )
	        face->set_boundary_id(1); // b_id1 face of periodic BC along y direction 

	      if (
		  (std::fabs(center(1) - half_length) < 1e-12 * half_length)
		 )
	        face->set_boundary_id(1); // b_id2 face of periodic BC along y direction 
	      
	      if (
		  (std::fabs(center(2) - (-half_length)) < 1e-12 * half_length)
		 )
	        face->set_boundary_id(1); // b_id1 face of periodic BC along z direction 
