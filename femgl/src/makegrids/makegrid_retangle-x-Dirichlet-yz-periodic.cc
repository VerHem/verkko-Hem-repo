  ...
  void FemGL<dim>::make_grid()
  {
	...
	for (const auto &cell : triangulation.cell_iterators())
	  for (const auto &face : cell->face_iterators())
		...
		face->set_boundary_id(0); // Derechlet BC along x direction 

	      if (std::fabs(center(1) - (-half_y_length)) < 1e-12 * half_y_length)
		...
		face->set_boundary_id(8); // b_id2 face of periodic BC along y direction 	    
	      ...
	      if (std::fabs(center(2) - (-half_z_length)) < 1e-12 * half_z_length)		 
	 ...
	 /* add periodicity info into triangulation*/
	 /*---------------------------------------*/
