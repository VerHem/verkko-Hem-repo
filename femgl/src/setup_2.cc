...
namespace FemGL_mpi
{
  ...
  void FemGL<dim>::setup_system()
  {
	...
	/*---------------------------------*/
        /* load B-phase ball radius ratio  */
	/*---------------------------------*/
        const double r_ratio    = conf.get_double("B-phase ball radius ratio");
	const double half_lengh = conf.get_double("cube half side length");
	const double r_B        = r_ratio * half_length;
	/*---------------------------------*/
        /* parameters loading ends at here */
	/*---------------------------------*/	
	
	VectorTools::interpolate(dof_handler,
                                 BinA<dim>(r),
				 distrubuted_tmp_solution
				 /*empty ComponentMask for all 18 components*/);
