...
#include "femgl.h"
#include "x_dirichlet_ABWallBox.h"
#include "dirichlet.h"
...
namespace FemGL_mpi
{
  ...
  void FemGL<dim>::setup_system()
  {
    ...
    std::vector<GridTools::PeriodicFacePair<typename DoFHandler<dim>::cell_iterator>> matched_pairs_y, matched_pairs_z;
    ...
    {
      /*---------------------------------------*/ 
      /*  loading x-Dirichlet BCs parameters   */
      /*---------------------------------------*/
      conf.enter_subsection("control parameters");
      const double x_axis_half_length = conf.get_double("half x length of retangle");	
      ...
      VectorTools::interpolate_boundary_values(dof_handler,
                                               0,  // 0 for all Dirichlet, 2 for diffuse, which 6 components are Dirichlet along x
                                               x_SurfaceABWall_BoundaryValues<dim>(x_axis_half_length, matelem_A, matelem_B),
	...
	const double x_ofA     = rangeA_ratio * x_axis_half_length;
	const double matelem_A = mat.gap_A_td(p, reduced_t) * 0.7071067811865475f;
	const double matelem_B = mat.gap_B_td(p, reduced_t) * 0.5773502691896258f;
	
        VectorTools::interpolate(dof_handler,
		                 BnA_x<dim>(x_ofA, p, reduced_t, matelem_A, matelem_B),
