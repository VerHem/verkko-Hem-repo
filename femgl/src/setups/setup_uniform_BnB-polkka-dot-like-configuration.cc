...
namespace FemGL_mpi
{
  ...
  void FemGL<dim>::setup_system()
  {
     ...
     if (cycle == 0)
       {
        ...
        const double radius_ratio          = conf.get_double("B-phase inner plate radius ratio");
        const double half_side_length      = conf.get_double("half x length of retangle");	
        conf.leave_subsection();	 
        /*---------------------------------------*/
	...
	const double radius    = radius_ratio * half_side_length;
