...
#include "femgl.h"
#include "z_dirichlet_ABWallBox.h"
#include "dirichlet.h"
 ...
 *
 * This setup must be used together with makegrids/makegrid_retangle-z-Dirichlet-xy-HomoNeumann.cc.
 * The make_grid() call in this source will generate a long 3D box along z-direction, and x-y direction 
 * boundaries all have boundary-id-1 a.k.a homo-Neumann. While z-surface has boundary-id 0 on all OP componenets.
 * These two z-surfaces will be put with bulk  A- and B-phase OP at p-T-H values.
 * 
...
namespace FemGL_mpi
{
  ...
  void FemGL<dim>::setup_system()
  {
    ...
    {            
       ...
       * the two flat boundary surfaces at z directon in ABWall-retangle box.
      ...
      VectorTools::interpolate_boundary_values(dof_handler,
                                               0, // 0 for all Dirichlet, 2 for diffuse, which 6 components are Dirichlet along x 
    ...
    {
      /*---------------------------------------*/ 
      /*  loading z-Dirichlet BCs parameters   */
      /*---------------------------------------*/
      ...
      // OP matrix prefectors i.e., gapA(B)/sqrt(2(3))
      ...
      VectorTools::interpolate_boundary_values(dof_handler,
                                               0,  // 0 for all Dirichlet, 2 for diffuse, which 6 components are Dirichlet along x
                                               z_SurfaceABWall_BoundaryValues<dim>(z_axis_half_length, matelem_A, matelem_B),
