...
namespace FemGL_mpi
{
  ...
  void FemGL<dim>::setup_system()
  {
     ...
     if (cycle == 0)
       {
        ...
        /*---------------------------------------*/ 
        /*  loading B-phase ball size paramters  */
        /*---------------------------------------*/
        conf.enter_subsection("control parameters");
        const double radius_ratio          = conf.get_double("B-phase ball radius ratio");
        const double cube_half_side_length = conf.get_double("cube half side length");	
	...
	const double radius    = radius_ratio * cube_half_side_length;
	...
	const double matelem_B = mat.gap_B_td(p, reduced_t) * 0.5773502691896258f;
        ...
        VectorTools::interpolate(dof_handler,
		                 BinA<dim>(radius, p, reduced_t, matelem_A, matelem_B),
