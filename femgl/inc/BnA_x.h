...
namespace FemGL_mpi
{
  ...
  template <int dim>
  class BnA_x : public Function<dim>
  {
  public:
    BnA_x(double xA, double p, double t, double matelem_A, double matelem_B)
                              ...
                              Vector<double> &values) const override
    {
      ...
      if ( point(0) >= rangeA )     // B-phase zone
	{
      ...
      else if ( point(0) < rangeA ) //outside sphere, A-phase
	{
	  for (unsigned int index = 0; index < values.size(); index++)
           {
	     if (
                      ...
                      std::vector<Vector<double>> &  value_list) const override
    {
      ...
      for (unsigned int p = 0; p < points.size(); ++p)
        BnA_x<dim>::vector_value(points[p], value_list[p]);
    }
  };
